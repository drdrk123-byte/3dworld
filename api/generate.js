...
const {generate,parseImage}=require('../lib/ai.cjs');const {createAuth}=require('../lib/worlds.cjs');
function createHandler({env=process.env,generateImpl=generate,fetchImpl=fetch}={}){const googleAuth=createAuth(env,fetchImpl);return async(req,res)=>{
 ...
 const tokenOk=secret.length>=32&&actual.length===expected.length&&crypto.timingSafeEqual(actual,expected);if(!tokenOk&&!await googleAuth(req))return send(401,{error:'교사용 Google 로그인이 필요하거나 만료되었습니다. 왼쪽 위 Google 로그인을 다시 눌러 주세요.'});
